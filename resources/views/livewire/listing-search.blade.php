<div class="py-10 sm:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 px-6 py-10 sm:px-10 sm:py-12 shadow-lg">
            ...
            <div class="relative">
                <p class="text-indigo-200 text-sm font-medium tracking-wide uppercase">{{ __('marketplace.tagline') }}</p>
                <h1 class="mt-2 text-3xl sm:text-4xl font-bold text-white tracking-tight">{{ __('marketplace.hero_title') }}</h1>
            ...
            <aside class="lg:col-span-3 mb-8 lg:mb-0">
                <div class="lg:sticky lg:top-6 bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50/80">
                        <h2 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">{{ __('marketplace.filters') }}</h2>
                    ...
                    <div class="p-5 space-y-5">
                        <div>
                            <x-input-label for="title" :value="__('marketplace.search')" />
                        ...
                        <div>
                            ...
                            <select
                                ...
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                            >
                                <option value="">{{ __('marketplace.all_countries') }}</option>
                        ...
                        <div>
                            <x-input-label :value="__('marketplace.price_range')" />
                            <div class="mt-1 grid grid-cols-2 gap-3">
                                <x-text-input
                                    ...
                                    placeholder="{{ __('marketplace.min') }}"
                                />
                                <x-text-input
                                    id="maxPrice"
                                    type="number"
                                    ...
                                    class="block w-full text-sm"
                                    placeholder="{{ __('marketplace.max') }}"
                                />
                            </div>
                        </div>

                        <div>
                            <x-input-label for="sort" :value="__('marketplace.sort_by')" />
                            <select
                                id="sort"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                    <div>
                        <p class="text-sm text-gray-500">
                            {{ trans_choice('marketplace.listings_found', $listings->total(), ['count' => number_format($listings->total())]) }}
                        </p>
                        @if ($listings->total() > 0)
                            <p class="text-xs text-gray-400 mt-0.5">
                                {{ __('marketplace.showing_range', ['from' => $listings->firstItem(), 'to' => $listings->lastItem()]) }}
                            </p>
                        @endif
                    </div>
                    ...
                    <div wire:loading.delay class="flex items-center gap-2 text-sm text-indigo-600">
                        ...
                        {{ __('marketplace.updating_results') }}
                    </div>
                </div>
                ...
                <div wire:loading.delay.class="opacity-60 pointer-events-none" class="transition-opacity duration-200">
                    @if ($listings->count() === 0)
                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-16 text-center">
                            ...
                            <h3 class="mt-4 text-base font-semibold text-gray-900">{{ __('marketplace.no_listings_match') }}</h3>
                            <p class="mt-2 text-sm text-gray-500 max-w-sm mx-auto">{{ __('marketplace.no_listings_hint') }}</p>
                            @if ($title || $category || $country || $minPrice || $maxPrice || $sort !== 'newest')
                                <button
                                    type="button"
                                    wire:click="clearFilters"
